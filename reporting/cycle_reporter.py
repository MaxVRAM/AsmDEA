...
class CycleReporter(BaseReporter):
    ...
    def print_console_report(self, report: CycleReport) -> None:
        ...
        panel = Panel(
            stats_table,
            title=f"Found {report.total_cycles} cyclic dependencies!",
            subtitle="[error]Cycle Detection[/]",
            border_style="red",
            expand=False,
        )
        console.print(panel)
        ...
        for i, cycle_detail in enumerate(report.cycles, 1):
            ...
            if self.detailed and cycle_detail.dependency_tree:
    ...
    # Helper to build Rich Tree from dependency data
    def _build_rich_tree(self, tree_data: dict[str, Any], parent: Tree | None = None) -> Tree:
        ...
        label = f"[cycle]X[/] [cycle]{name}[/]" if in_cycle else f"[no_cycle]o[/] [no_cycle]{name}[/]"
